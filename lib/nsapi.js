...
class ServerApiClient {
    ...
    async apiFormPut(path, filePath, successFunction) {
        if (typeof successFunction !== 'function') successFunction = () => { };
        const url = `https://${this.hostname}/ns-api/v2/${path}`;
        ...
        try {
            ...
            if (error.code === 'ECONNABORTED') {
                console.error(`[${this.serverId}] apiFormPut timeout: ${url}`);
            } else {
                console.log(error);
            }
    ...
    async apiCountSync(path) {
        ...
        const makeRequest = async () => {
            ...
            if (this.debug) console.log(`[${this.serverId}]`, options.method, url);
            return this.axiosInstance.request(options);
        };
        ...
        try {
            ...
            console.error(`[${this.serverId}] COUNT ERROR url - ${url}`);
            if (error?.response?.status) {
                console.error(`HTTP ${error.response.status}: ${error.response.statusText}`);
                if (error.response.data) {
                    console.error("Response data:", error.response.data);
