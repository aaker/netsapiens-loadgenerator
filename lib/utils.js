...
function addToCsv(data, serverId = null) {
  ...
  // would all pass the duplicate check before any write landed (TOCTOU race).
  // Sync ops run atomically within the Node.js event loop tick.
  // field4 = SIP User-Agent. Defaults to "netsapiens-loadgenerator" when none is supplied.
  const userAgent = data.userAgent || 'netsapiens-loadgenerator';
  const stringData = `${data.displayName};${data.device};${data.domain};[authentication username=${data.device} password=${data['device-sip-registration-password']}];${userAgent}`;
  const filedata = fs.readFileSync(FILE_LOCATION);
