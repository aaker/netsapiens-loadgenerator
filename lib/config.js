  ...
  _loadMultiServerConfig(filePath) {
    try {
      ...
      this.servers = config.servers.map(server => ({
        ...
        ndpServerName: server.ndpServerName || process.env.NDP_SERVERNAME || 'core1',
        evenResellerDistribution: server.evenResellerDistribution || false,
        domainSizeHardLimit: server.domainSizeHardLimit || null
